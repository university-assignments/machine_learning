Чтобы исправить ошибку, заменим устаревший датасет Boston Housing на California Housing. Вот исправленный код:
...
Основные изменения:
1. Заменен `load_boston` на `fetch_california_housing`
2. Исправлены названия графиков для соответствия новому датасету
3. Упрощен вывод метрик с помощью метода `.fit()` в цепочке вызовов

California Housing Dataset имеет:
- 20 640 образцов
- 8 признаков
- Непрерывный целевой признак (стоимость дома)
- Актуален для задач регрессии
- Не содержит этически спорных данных
